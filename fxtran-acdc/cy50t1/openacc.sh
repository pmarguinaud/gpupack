...
for f in \
   arpifs/phys_dmn/arotherdia.F90                  \
...
#  --only-if-newer 
  openacc.pl --interface --stack84 \
   ...
   --nocompute ABOR1 --version --cycle 50 \
   $(resolve --user $f)
done
...
dir=$(dirname $f)
mkdir -p src/local/ifsaux/openacc/$dir
openacc.pl --interface --stack84 --cycle 50 --pointers --nocompute ABOR1 --version --cpg_dyn --dir src/local/ifsaux/openacc/$dir $(resolve $f)
