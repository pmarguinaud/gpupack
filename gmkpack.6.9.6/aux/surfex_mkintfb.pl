...
# pour imprimer :
#my $log = 'FileHandle'->new (">/tmp/mkintfb.log");
#$log->autoflush (1);
# puis :        $log->print ("coucou\n");
...
#   ----------------------------------------------------------------------------
	...
	find ({wanted => sub { $f{basename($File::Find::name)}++ }, no_chdir => 1}, $prj);
        for my $intfb (<$GMKINTFB/$prj/modi_*.F90>)
	  {
            my $f2 = basename ($intfb);
            my $f = ($f2 =~ s/^modi_//);
	    $f{$f2} or do
	      {
                unlink ($intfb);
...
if (@all)
  {
    ...
    find ({wanted => sub { m/modi_.*\.F90/o && $fh->print ("$File::Find::name\n") }, no_chdir => 1}, 
	  $GMKINTFB);
  }
...
sub make_intfbl1
{
 ...
 # Read in lines from file
    ...
    unless($prog_info{is_module}) 
      {
        $fname=~s#.*/(.+)$#$1#;
        &create_interface_module(\@statements,\@interface_block,$fname);
        &cont_lines_surfex (\@interface_block,\@lines,\@line_hash);
        my $int_block_fname = $fname;
        $int_block_fname =~ s/^/modi_/;
