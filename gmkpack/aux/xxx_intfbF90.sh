...
export GMK_VOB=xxx
$GMKROOT/aux/xxx_check_inc_intfb.pl $1 1> intfb_report 2>&1
if [ $? -eq 0 ] ; then
...
#     Anyway, the compilation will fail later on if this auto-generated interface is needed.
      if [ $(find $MKTOP/*/$GMKINTFB/* -name "${subroutine}.intfb.h" -type f -print | wc -l) -ne 0 ] ; then
  ...
  for subroutine in $(comm -13 include calls) ; do
    if [ $(grep -c ${subroutine}.h explicit) -eq 0 ] ; then
      if [ $(find $MKTOP/*/$GMKINTFB/* -name "${subroutine}.intfb.h" -print | wc -l) -ne 0 ] ; then
        echo MISSING INTERFACE BLOCK - ADD LINE : \#include \"${subroutine}.intfb.h\" >> intfb_fatal
      fi
    fi
  done
